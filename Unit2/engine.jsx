...
// All reusable renderers (KaTeX, SVG diagrams, every slide-type component incl.
// RealGasSlide), totalSteps(), and the <Presentation> shell.
// Content lives in ./slides/*; styles live in ./styles.js.
// <Presentation> takes `slides`, `meta`, and (optional) `sections` props.
...
function StepGraph({ lit, accent, accent2, ink, muted }) {
  ...
  // map a cv/R value (1.5, 2.5, 3.5) to a y pixel
...
// ─── Animated cp/cv concept illustration ────────────────────────────────────
function PistonPair({ accent, accent2, ink, muted }) {
  const [run, setRun] = useState(0)
  const replay = () => setRun(r => r + 1)
  return (
    <svg viewBox="0 0 540 230" className="cpcv-svg" aria-label="Constant-volume tank, constant-pressure piston cylinder, and thermometer impacted by gas molecules"
      onMouseEnter={replay} onClick={(e) => { e.stopPropagation(); replay() }}>
      <g key={run}>
        {/* constant volume: sealed rigid tank */}
        <g className="cv-vessel">
          <text x="28" y="126" className="cpcv-flame">🔥</text>
          <line x1="48" y1="118" x2="73" y2="105" stroke={accent2} strokeWidth="3" markerEnd="url(#cp-arrow)" />
          <rect x="76" y="54" width="92" height="104" rx="5" fill="rgba(94,200,216,.08)" stroke={ink} strokeWidth="2.6" />
          <text x="122" y="184" textAnchor="middle" className="cpcv-cap">Constant volume tank</text>
          <text x="122" y="203" textAnchor="middle" className="cpcv-subcap">energy → molecular motion</text>
          <g className="cv-shimmer">
            <circle cx="101" cy="91" r="4" fill={accent} />
            <circle cx="136" cy="80" r="4" fill={accent} />
            <circle cx="144" cy="123" r="4" fill={accent} />
            <circle cx="106" cy="135" r="4" fill={accent} />
            <path d="M101 91 l18 -11" stroke={accent} strokeWidth="1.6" markerEnd="url(#mol-arrow)" />
            <path d="M136 80 l16 22" stroke={accent} strokeWidth="1.6" markerEnd="url(#mol-arrow)" />
            <path d="M144 123 l-26 14" stroke={accent} strokeWidth="1.6" markerEnd="url(#mol-arrow)" />
          </g>
          <text x="82" y="42" className="cpcv-small">cᵥ: no boundary work</text>
        </g>

        {/* constant pressure: free piston rises against load */}
        <g className="cp-vessel">
          <text x="193" y="126" className="cpcv-flame">🔥</text>
          <line x1="213" y1="118" x2="238" y2="105" stroke={accent2} strokeWidth="3" markerEnd="url(#cp-arrow)" />
          <path d="M 242 48 L 242 158 L 328 158 L 328 48" fill="rgba(94,200,216,.08)" stroke={ink} strokeWidth="2.6" />
          {[256, 274, 292, 310].map((x, i) => (
            <line key={i} x1={x} y1="20" x2={x} y2="37" stroke={accent2} strokeWidth="1.8" markerEnd="url(#cp-load)" />
          ))}
          <text x="285" y="14" textAnchor="middle" className="cpcv-small">load / atmosphere</text>
          <g className="cp-piston">
            <rect x="242" y="60" width="86" height="10" fill={muted} stroke={ink} strokeWidth="1.2" />
          </g>
          <rect className="cp-gas" x="244" y="71" width="82" height="86" fill="rgba(94,200,216,.18)" stroke={accent} strokeWidth="1" />
          <g className="cv-shimmer">
            <circle cx="264" cy="112" r="4" fill={accent} />
            <circle cx="299" cy="101" r="4" fill={accent} />
            <circle cx="304" cy="139" r="4" fill={accent} />
            <path d="M264 112 l25 -10" stroke={accent} strokeWidth="1.6" markerEnd="url(#mol-arrow)" />
            <path d="M299 101 l13 26" stroke={accent} strokeWidth="1.6" markerEnd="url(#mol-arrow)" />
          </g>
          <text x="285" y="184" textAnchor="middle" className="cpcv-cap">Constant pressure cylinder</text>
          <text x="285" y="203" textAnchor="middle" className="cpcv-subcap">energy → motion + work</text>
          <text x="247" y="42" className="cpcv-small">cₚ: piston work too</text>
        </g>

        {/* thermometer: molecular collisions sensed as temperature */}
        <g className="thermo-demo">
          <text x="408" y="26" className="cpcv-small">What the sensor feels</text>
          <rect x="444" y="58" width="18" height="94" rx="9" fill="rgba(255,255,255,.06)" stroke={ink} strokeWidth="2" />
          <circle cx="453" cy="164" r="20" fill="rgba(255,255,255,.06)" stroke={ink} strokeWidth="2" />
          <rect x="450" y="79" width="6" height="79" rx="3" fill={accent2} />
          <circle cx="453" cy="164" r="13" fill={accent2} />
          <line x1="464" y1="74" x2="474" y2="74" stroke={muted} strokeWidth="1.5" />
          <line x1="464" y1="98" x2="474" y2="98" stroke={muted} strokeWidth="1.5" />
          <line x1="464" y1="122" x2="474" y2="122" stroke={muted} strokeWidth="1.5" />
          <g className="temp-molecules">
            <circle cx="386" cy="75" r="4" fill={accent} />
            <circle cx="398" cy="137" r="4" fill={accent} />
            <circle cx="504" cy="92" r="4" fill={accent} />
            <circle cx="516" cy="151" r="4" fill={accent} />
            <path d="M386 75 L438 91" stroke={accent} strokeWidth="1.8" markerEnd="url(#mol-arrow)" />
            <path d="M398 137 L435 153" stroke={accent} strokeWidth="1.8" markerEnd="url(#mol-arrow)" />
            <path d="M504 92 L468 111" stroke={accent} strokeWidth="1.8" markerEnd="url(#mol-arrow)" />
            <path d="M516 151 L475 159" stroke={accent} strokeWidth="1.8" markerEnd="url(#mol-arrow)" />
          </g>
          <text x="453" y="202" textAnchor="middle" className="cpcv-subcap">moving molecules impact mercury bulb</text>
        </g>

        <defs>
          <marker id="cp-arrow" markerWidth="10" markerHeight="10" refX="8" refY="4" orient="auto" markerUnits="strokeWidth">
            <path d="M0,0 L8,4 L0,8 Z" fill={accent2} />
          </marker>
          <marker id="cp-load" markerWidth="8" markerHeight="8" refX="5" refY="3" orient="auto" markerUnits="strokeWidth">
            <path d="M0,0 L6,3 L0,6 Z" fill={accent2} />
          </marker>
          <marker id="mol-arrow" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto" markerUnits="strokeWidth">
            <path d="M0,0 L6,3 L0,6 Z" fill={accent} />
          </marker>
        </defs>
      </g>
...
function CpCvSlide({ slide, revealed }) {
  const accent = '#5ec8d8', accent2 = '#f0a93b', ink = '#eaf1f8', muted = '#8da4be'
  return (
    <div className="slide-inner compress-slide cpcv-slide">
      <div className="section-number anim-in">{slide.sectionNumber}</div>
      <h2 className="slide-heading anim-in"><HTML>{slide.heading}</HTML></h2>
      <div className="heading-rule anim-in" />

      <div className={`reveal-block${revealed > 0 ? ' revealed' : ''}`}>
        <p className="cf-note cf-note--lead cpcv-lead"><HTML>{slide.setup}</HTML></p>
      </div>
      ...
      <div className="cpcv-layout">
        ...
        <div className="cpcv-explain-stack">
          <div className={`reveal-block cpcv-concept${revealed > 2 ? ' revealed' : ''}`}>
            <p className="cf-note"><HTML>{slide.temperature}</HTML></p>
          </div>
          <div className={`reveal-block cpcv-concept${revealed > 3 ? ' revealed' : ''}`}>
            <p className="cf-note"><HTML>{slide.pressure}</HTML></p>
          </div>
          <div className={`reveal-block cpcv-work${revealed > 4 ? ' revealed' : ''}`}>
      ...
      <div className="rel-row">
        {slide.relations.map((r, i) => (
          <div key={i} className={`rel-card${revealed > i + 5 ? ' revealed' : ''}`}>
      ...
      <div className={`reveal-block cf-bridge${revealed > 7 ? ' revealed' : ''}`}>
  ...
  const examples = [
    {
      ...
      body: 'Adding heat δq to a gas at temperature T increases entropy by dS = δq/T. In a real combustor, '
        + 'additional irreversibilities (viscous dissipation, mixing) raise entropy further — so some mechanical '
        + 'usefulness of the flow is lost.',
    },
    {
      icon: '🌊', title: 'Sudden compression',
      body: 'If a compression happens irreversibly, ordered mechanical energy is degraded into thermal disorder. '
        + 'The gas can end at a higher entropy state even if no heat crosses the boundary.',
    },
    {
    ...
    {
      icon: '⚙️', title: 'Wall friction in ducts',
      body: 'Viscous friction converts organized kinetic energy into molecular-scale thermal motion. '
        + 'That energy remains in the flow, but in a less recoverable form, so entropy increases.',
    },
  ]
  return (
    <div className="slide-inner compress-slide">
...
function ExampleSlide({ slide, revealed }) {
  return (
    <div className="slide-inner compress-slide example-slide">
      <div className="section-number anim-in">{slide.sectionNumber}</div>
      <div className="ex-callout anim-in">In-class example</div>
      <h2 className="slide-heading anim-in"><HTML>{slide.heading}</HTML></h2>
      <div className="heading-rule anim-in" />
      ...
      <div className="ex-steps">
        {(slide.steps || []).slice(0, revealed).map((step, i) => (
          <div key={i} className="ex-step revealed">
...
// ─── Real-gas molecular sketches (high-P packing vs low-T attraction) ────────
function MoleculeBox({ kind, accent }) {
  const muted = '#8da4be'
  if (kind === 'dense') {
    // packed grid — "molecular volume matters" at high pressure
    const cols = 6, rows = 4, x0 = 22, y0 = 24, dx = 33, dy = 26, r = 12
    const dots = []
    for (let j = 0; j < rows; j++) {
      for (let i = 0; i < cols; i++) {
        dots.push(
          <circle key={`d${i}-${j}`} cx={x0 + i * dx} cy={y0 + j * dy} r={r}
            fill={accent} fillOpacity="0.55" stroke={accent} strokeWidth="1.2" />
        )
      }
    }
    return (
      <svg viewBox="0 0 216 134" className="mol-svg" aria-hidden>
        <rect x="6" y="6" width="204" height="122" rx="5" fill="none" stroke={muted} strokeWidth="1.5" />
        {dots}
      </svg>
    )
  }
  // "forces" — sparse molecules with dashed attraction lines, low temperature
  const mol = [ [54, 46], [104, 80], [150, 44], [122, 110], [66, 104] ]
  const bonds = [ [0, 1], [1, 2], [1, 3], [1, 4] ]
  return (
    <svg viewBox="0 0 216 134" className="mol-svg" aria-hidden>
      <rect x="6" y="6" width="204" height="122" rx="5" fill="none" stroke={muted} strokeWidth="1.5" />
      {bonds.map(([a, b], i) => (
        <line key={`b${i}`} x1={mol[a][0]} y1={mol[a][1]} x2={mol[b][0]} y2={mol[b][1]}
          stroke={accent} strokeWidth="1.4" strokeDasharray="3 4" opacity="0.85" />
      ))}
      {mol.map(([cx, cy], i) => (
        <g key={`m${i}`}>
          <circle cx={cx} cy={cy} r="16" fill="none" stroke={accent} strokeWidth="1"
            strokeDasharray="2 4" opacity="0.5" />
          <circle cx={cx} cy={cy} r="11" fill={accent} fillOpacity="0.55" stroke={accent} strokeWidth="1.2" />
        </g>
      ))}
    </svg>
  )
...
function RealGasSlide({ slide, revealed }) {
  return (
    <div className="slide-inner compress-slide">
      <div className="section-number anim-in">{slide.sectionNumber}</div>
      <h2 className="slide-heading anim-in"><HTML>{slide.heading}</HTML></h2>
      ...
      <div className="cmp-row">
        {slide.regimes.map((r, i) => (
          <div key={i} className={`regime-card reveal-block${revealed > i + 1 ? ' revealed' : ''}`}
            ...
            <MoleculeBox kind={r.diagram} accent={r.accent} />
            <strong className="regime-headline" style={{ color: r.accent }}><HTML>{r.head}</HTML></strong>
            <p className="regime-body"><HTML>{r.body}</HTML></p>
          </div>
        ))}
...
function totalSteps(slide) {
  switch (slide.type) {
    ...
    case 'cpcv':        return 8 // setup, visual, temperature, pressure, work, PG, CPG, note
    ...
    case 'realgas':    return 4 // intro, 2 panels, closer
    default:            return 0
  }
}
...
export default function Presentation({ slides: slideData, meta: metaData, sections = [] }) {
  ...
  function renderSlide(slide) {
    switch (slide.type) {
      ...
      case 'realgas':     return <RealGasSlide slide={slide} revealed={revealed} />
      default:            return null
    }
  }
