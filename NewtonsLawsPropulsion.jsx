...
// Live polling (see the setup block lower down). Requires: npm install firebase
...
//   2. Create a free Firebase project → add a Realtime Database (start in test
//      mode), then paste its config below.
//   3. Deploy the deck. Students scan the QR shown on a question slide (or open
...
function usePollVotes(sessionCode, questionId) {
  ...
  useEffect(() => {
    ...
    const offVotes = onValue(ref(pollDb, `${base}/votes`), (s) => setVotes(s.val() || {}));
    const offOpen = onValue(ref(pollDb, `${base}/open`), (s) => setOpen(Boolean(s.val())));
    return () => { offVotes(); offOpen(); };
  }, [sessionCode, questionId]);
  return { votes, open };
}

function setPollOpen(questionId, isOpen) {
  if (pollEnabled) set(ref(pollDb, `sessions/${POLL_SESSION}/${questionId}/open`), isOpen);
}
function resetPollVotes(questionId) {
  if (pollEnabled) remove(ref(pollDb, `sessions/${POLL_SESSION}/${questionId}/votes`));
}
function castPollVote(sessionCode, questionId, optionIndex) {
  if (pollEnabled) set(ref(pollDb, `sessions/${sessionCode}/${questionId}/votes/${pollDeviceId()}`), optionIndex);
}
...
function StudentVote({ code }) {
  ...
  useEffect(() => {
    if (!pollEnabled) return;
    const ids = Object.keys(conceptQuestions);
    const offs = ids.map((id) =>
      onValue(ref(pollDb, `sessions/${code}/${id}/open`), (s) =>
        setOpenMap((m) => ({ ...m, [id]: Boolean(s.val()) }))
      )
    );
