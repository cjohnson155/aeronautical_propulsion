...
function PressureDiagram() {
  ...
  const N = 14;
  const ARROW = 34;   // external arrow length (outside → skin)
  const INT_ARROW = 34; // internal arrow length (centre → skin)
  ...
  // suppress external arrows near open neck in step >= 1
  const extOk = (p) => step === 0 ? true : p.nx < 0.55;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12, width: "100%" }}>
      ...
      <svg viewBox="0 0 680 310" style={{ width: "100%", maxWidth: 680, height: "auto", overflow: "visible" }}>
        <defs>
          {/* External: arrowhead at END, tip points inward (toward skin) — standard forward marker */}
          <marker id="pd-ext" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto">
            <path d="M0,0 L0,8 L8,4 z" fill="#00d4ff" />
          </marker>
          {/* Internal: arrowhead at END, tip points outward (toward skin) — standard forward marker */}
          <marker id="pd-int" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto">
            <path d="M0,0 L0,8 L8,4 z" fill="#ff6b35" />
          </marker>
          {/* Net-force arrow: large, orange, forward */}
          <marker id="pd-fnet" markerWidth="12" markerHeight="12" refX="11" refY="6" orient="auto">
        ...
        {step < 2 && (
          <>
            ...
            {showExt && allPts.map((p, i) => {
              ...
              return (
                <line key={`ext-${i}`} x1={x1} y1={y1} x2={x2} y2={y2}
                  stroke="#00d4ff" strokeWidth="2.2" markerEnd="url(#pd-ext)"
                  filter="url(#glow-b)"
            ...
            {showInt && allPts.map((p, i) => {
              ...
              return (
                <line key={`int-${i}`} x1={x1} y1={y1} x2={x2} y2={y2}
                  stroke="#ff6b35" strokeWidth="2.2" markerEnd="url(#pd-int)"
                  filter="url(#glow-o)"
                  style={{ opacity: 0, animation: `pdFade 0.3s ${i*0.04}s ease forwards` }} />
              );
            })}

            {/* Prominent horizontal arrows on equator (both sides) */}
            {showInt && (
              <>
                {/* LEFT: external inward */}
                {showExt && (
                  <line x1={CX - RX - ARROW - 4} y1={CY} x2={CX - RX - 3} y2={CY}
                    stroke="#00d4ff" strokeWidth="3.5" markerEnd="url(#pd-ext)"
                    style={{ opacity: 0, animation: "pdFade 0.4s 0.6s ease forwards" }} />
                )}
                {/* LEFT wall internal outward push (→ right along left interior) */}
                <line x1={CX - RX + INT_ARROW} y1={CY} x2={CX - RX + 3} y2={CY}
                  stroke="#ff6b35" strokeWidth="3.5" markerEnd="url(#pd-int)"
                  style={{ opacity: 0, animation: "pdFade 0.4s 0.65s ease forwards" }} />

                {/* RIGHT: external inward (only in sealed step) */}
                {showExt && step === 0 && (
                  <line x1={CX + RX + ARROW + 4} y1={CY} x2={CX + RX + 3} y2={CY}
                    stroke="#00d4ff" strokeWidth="3.5" markerEnd="url(#pd-ext)"
                    style={{ opacity: 0, animation: "pdFade 0.4s 0.6s ease forwards" }} />
                )}
                {/* RIGHT wall internal outward push (→ right, only sealed) */}
                {step === 0 && (
                  <line x1={CX + RX - INT_ARROW} y1={CY} x2={CX + RX - 3} y2={CY}
                    stroke="#ff6b35" strokeWidth="3.5" markerEnd="url(#pd-int)"
                    style={{ opacity: 0, animation: "pdFade 0.4s 0.65s ease forwards" }} />
            ...
            {/* Layer 3: Skin tension ticks (step 0 only) */}
            {showSkin && skinPts.map((p, i) => {
              const tx = -p.ny, ty = p.nx, T = 8;
              return (
                <line key={`sk-${i}`}
            ...
            {step === 1 && (
              <>
                {/* Projected area dashed box on left wall */}
                <rect x={CX-RX-8} y={NY - RY*0.6} width="12" height={RY*1.2}
                  fill="#ff6b3515" stroke="#ff6b35" strokeWidth="1.5" strokeDasharray="4 3" rx="2"
                  style={{ opacity: 0, animation: "pdFade 0.5s 0.3s ease forwards" }} />
                <text x={CX-RX-22} y={NY-RY*0.6-8} fill="#ff6b35" fontSize="9" fontFamily="monospace" textAnchor="middle"
                  style={{ opacity: 0, animation: "pdFade 0.4s 0.4s ease forwards" }}>A_proj</text>

                {/* Net force arrow — LEFT direction: start right of centre, end left, tip at left */}
                <line x1={CX+50} y1={CY+RY+32} x2={CX-90} y2={CY+RY+32}
                ...
                <text x={CX-20} y={CY+RY+58} fill="#ff6b35" fontSize="12" fontFamily="monospace" textAnchor="middle" fontWeight="bold"
                  style={{ opacity: 0, animation: "pdFade 0.5s 1.6s ease forwards" }}>
                  F_net = (P_int − P_atm) × A_proj
                </text>
                <text x={CX-20} y={CY+RY+74} fill="#8b949e" fontSize="10" fontFamily="monospace" textAnchor="middle"
                  style={{ opacity: 0, animation: "pdFade 0.5s 1.9s ease forwards" }}>
                  right wall missing → no cancellation → balloon accelerates left
                </text>
              </>
            )}
