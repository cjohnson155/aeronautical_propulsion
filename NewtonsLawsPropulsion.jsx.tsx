...
// ── Pressure Diagram ───────────────────────────────────────────────────────
// Helpers: compute N uniformly-spaced points on an ellipse and return outward unit normals.
function ellipseArrows(cx, cy, rx, ry, n, startAngle = 0) {
  return Array.from({ length: n }, (_, i) => {
    const t = startAngle + (2 * Math.PI * i) / n;
...
function PressureDiagram() {
  ...
  // Balloon geometry — viewBox 0 0 620 270
  const CX = 260, CY = 135, RX = 128, RY = 86;
  const N = 14;
  const ARROW = 32, INT_ARROW = 26;

  const allPts  = ellipseArrows(CX, CY, RX, RY, N);
  const skinPts = ellipseArrows(CX, CY, RX, RY, 18);

  // nozzle is on the RIGHT side of the balloon
  const NX = CX + RX, NY = CY; // nozzle centre

  // For step>=1, suppress external arrows near the open neck (right side)
  const extOk = (p) => {
    if (step === 0) return true;
    // hide arrows where outward normal points right (nozzle region)
    return p.nx < 0.55;
  };
  ...
  const layerMeta = [
    { label: "Layer 1: External P_atm (inward)", color: "#00d4ff" },
    { label: "Layer 2: Internal P_int (outward, higher)", color: "#ff6b35" },
    { label: "Layer 3: Skin tension (tangential)", color: "#a8ff3e" },
  ];
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12, width: "100%" }}>
      ...
      {/* Layer buttons (only for steps 0 & 1) */}
      {step < 2 && (
      ...
      <svg viewBox="0 0 620 270" style={{ width: "100%", maxWidth: 640, height: "auto", overflow: "visible" }}>
        <defs>
          <marker id="pd-ext" markerWidth="7" markerHeight="7" refX="6" refY="3.5" orient="auto">
            <path d="M0,0 L0,7 L7,3.5 z" fill="#00d4ff" />
          </marker>
          <marker id="pd-int" markerWidth="7" markerHeight="7" refX="1" refY="3.5" orient="auto">
            <path d="M7,0 L7,7 L0,3.5 z" fill="#ff6b35" />
          </marker>
          <marker id="pd-orange-big" markerWidth="10" markerHeight="10" refX="1" refY="5" orient="auto">
            <path d="M10,0 L10,10 L0,5 z" fill="#ff6b35" />
          </marker>
          <marker id="pd-exhaust" markerWidth="8" markerHeight="8" refX="6" refY="4" orient="auto">
        ...
        {/* ══ STEP 3: balloon moves left, air exhausts right ══ */}
        {step === 2 && (
          <>
            {/* Background hint text */}
            <text x="310" y="22" fill="#555" fontSize="10" fontFamily="monospace" textAnchor="middle">Newton III: action = exhaust right, reaction = balloon left</text>
            ...
            <g style={{
              ...
              {/* motion arrow on balloon */}
              <line x1={CX - 20} y1={CY - RY - 18} x2={CX - 80} y2={CY - RY - 18}
                stroke="#ff6b35" strokeWidth="3" markerEnd="url(#pd-orange-big)"
                style={{ opacity: step3t ? 1 : 0, transition: "opacity 0.6s 0.9s" }}
              />
              <text x={CX - 50} y={CY - RY - 26} fill="#ff6b35" fontSize="12" fontFamily="monospace" textAnchor="middle"
                style={{ opacity: step3t ? 1 : 0, transition: "opacity 0.6s 0.9s" }}>balloon ←</text>
            </g>

            {/* Exhaust plume going right */}
            <g style={{
              opacity: step3t ? 1 : 0,
              ...
              {[0,1,2,3,4].map(i => (
                <line key={i}
                  x1={NX + 22 + i * 16} y1={NY + (i % 2 === 0 ? -4 : 4) * i * 0.3}
                  x2={NX + 32 + i * 16} y2={NY + (i % 2 === 0 ? -6 : 6) * i * 0.3}
                  stroke="#a8ff3e" strokeWidth={2 - i * 0.25} markerEnd="url(#pd-exhaust)" opacity={1 - i * 0.15}
                />
              ))}
              <text x={NX + 90} y={NY + 4} fill="#a8ff3e" fontSize="11" fontFamily="monospace">→ air</text>
            </g>
          </>
        )}

        {/* ══ STEPS 0 & 1: pressure physics diagram ══ */}
        {step < 2 && (
          <>
            {/* ── LAYER 1: External arrows (drawn first, appear behind balloon) ── */}
            {showExt && allPts.map((p, i) => {
              if (!extOk(p)) return null;
              const x1 = p.x + p.nx * ARROW;
            ...
            {/* ── Balloon body — drawn on top of ext arrows ── */}
            <ellipse cx={CX} cy={CY} rx={RX} ry={RY}
              fill="#111428" fillOpacity="0.92" stroke="#ff6b35" strokeWidth="2.5" />

            {/* ── LAYER 2: Internal arrows (outward, drawn ON TOP of balloon) ── */}
            {showInt && allPts.map((p, i) => {
              ...
              return (
                <line key={`int-${i}`} x1={x1} y1={y1} x2={x2} y2={y2}
                  stroke="#ff6b35" strokeWidth="2" markerEnd="url(#pd-int)"
                  filter="url(#glow-o)"
                  style={{ opacity: 0, animation: `pdFade 0.3s ${i*0.04}s ease forwards` }}
                />
              );
            ...
            {/* ── LAYER 3: Skin tension ticks (on top of balloon) ── */}
            ...
            {/* ── STEP 1 extras: projected area box + unbalanced force ── */}
            {step === 1 && (
              <>
                {/* Projected area rectangle on LEFT side (opposite neck) */}
                <rect
                  x={CX - RX - 2} y={NY - RY * 0.55}
                  width="10" height={RY * 1.1}
                  fill="#ff6b3520" stroke="#ff6b35" strokeWidth="1.5" strokeDasharray="4 3"
                  rx="2"
                />
                <text x={CX - RX - 28} y={NY - RY * 0.55 - 6}
                  fill="#ff6b35" fontSize="9.5" fontFamily="monospace" textAnchor="middle">A_proj</text>
                <text x={CX - RX - 28} y={NY - RY * 0.55 + 6}
                  fill="#ff6b35" fontSize="9.5" fontFamily="monospace" textAnchor="middle">(projected</text>
                <text x={CX - RX - 28} y={NY - RY * 0.55 + 17}
                  fill="#ff6b35" fontSize="9.5" fontFamily="monospace" textAnchor="middle">area)</text>

                {/* Net force arrow — animates in LAST (long delay) */}
                <line
                  x1={CX + 30} y1={CY + RY + 28}
                  x2={CX - 70} y2={CY + RY + 28}
                  stroke="#ff6b35" strokeWidth="5.5" markerEnd="url(#pd-orange-big)"
                  style={{ opacity: 0, animation: "pdFade 0.6s 1.2s ease forwards" }}
                />
                <text x={CX - 30} y={CY + RY + 48}
                  fill="#ff6b35" fontSize="13" fontFamily="monospace" fontWeight="bold" textAnchor="middle"
                  style={{ opacity: 0, animation: "pdFade 0.5s 1.5s ease forwards" }}>
                  F_net ← (= P_int × A_proj)
                </text>
              </>
            )}
      ...
      <p style={{ color: "#aaa", fontFamily: "'DM Mono', monospace", fontSize: 11, textAlign: "center", maxWidth: 560, margin: 0 }}>
        {stepMeta[step].label}
        {step < 2 && layer > 0 && ` · ${layerMeta[layer-1].label}`}
      </p>
...
function InternalPerspectiveDiagram() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12, width: "100%" }}>
      ...
      <svg viewBox="0 0 620 270" style={{ width: "100%", maxWidth: 640, height: "auto", overflow: "visible" }}>
        <defs>
          <marker id="ip-int" markerWidth="7" markerHeight="7" refX="1" refY="3.5" orient="auto">
            <path d="M7,0 L7,7 L0,3.5 z" fill="#ff6b35" />
          </marker>
          <marker id="ip-big" markerWidth="10" markerHeight="10" refX="1" refY="5" orient="auto">
            <path d="M10,0 L10,10 L0,5 z" fill="#ff6b35" />
          </marker>
        ...
        {step === 1 && (
          <>
            ...
            {/* Net force arrow → left */}
            <line x1={CX + 30} y1={CY + RY + 28} x2={CX - 80} y2={CY + RY + 28}
              stroke="#ff6b35" strokeWidth="5.5" markerEnd="url(#ip-big)"
              style={{ opacity: 0, animation: "pdFade 0.6s 1s ease forwards" }}
            />
            <text x={CX - 25} y={CY + RY + 47} fill="#ff6b35" fontSize="12" fontFamily="monospace" textAnchor="middle" fontWeight="bold"
        ...
        {step === 2 && (
          <>
            <g style={{
              ...
              {/* Balloon motion arrow */}
              <line x1={CX - 20} y1={CY - RY - 16} x2={CX - 80} y2={CY - RY - 16}
