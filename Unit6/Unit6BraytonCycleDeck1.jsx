...
export const slides = [
  ...
  // ── FLOW-PATH TOUR · MOTIVATION ─────────────────────────────────────────────
  {
    type: 'concept',
    sectionNumber: 'Flow-Path Tour',
    heading: 'Modeling the Flow, Component by Component',
    intro:
      'We\u2019ve traced how energy changes form through each component. The next question is how to <em>model the flow itself</em> \u2014 so we can put real numbers on it. For every station we settle on a short list of justifiable assumptions that let us reuse the tools built up in Units 2\u20134.',
    cards: [
      { tag: '\u03b7', accent: C_THERM, label: 'Component efficiency',
        body: 'Pin down how close each component runs to its ideal, so we can score it later.' },
      { tag: 'w', accent: C_MECH, label: 'Work bookkeeping',
        body: 'Track how much work we <strong>add</strong> at the compressor and <strong>extract</strong> at the turbine.' },
      { tag: '\u2192', accent: C_CHEM, label: 'Mission optimization',
        body: 'Choose design points that tune the engine for the <strong>particular mission</strong> it must fly.' },
    ],
    bridge:
      'Take the components in flow order \u2014 inlet, compressor, burner, turbine, nozzle \u2014 and justify the assumptions that unlock those tools.',
  },
...
// few extra interior knots to shape its two combustion spikes.)
const ENGINE_CURVES = [
  { cls: 'q6-chem',  pts: [[60,280],[213.75,280],[238,250],[252,175],[265,124.4],[278,175],[292,250],[316.25,280],[367.5,280],[392,255],[406,195],[418.75,157.8],[432,195],[446,250],[470,280]] },
  { cls: 'q6-therm', pts: [[60,235.6],[111.25,217.8],[162.5,191.1],[213.75,142.2],[265,57.8],[316.25,120],[367.5,164.4],[418.75,106.7],[470,180]] },
  { cls: 'q6-mech',  pts: [[60,231.1],[111.25,206.7],[162.5,175.6],[213.75,106.7],[265,108.9],[316.25,160],[367.5,200],[418.75,204.4],[470,235.6]] },
  { cls: 'q6-kin',   pts: [[60,195.6],[111.25,204.4],[162.5,206.7],[213.75,204.4],[265,213.3],[316.25,191.1],[367.5,182.2],[418.75,191.1],[470,66.7]] },
]
...
// Energy through the engine. All four currencies advance together, but one
// component at a time: each station interval draws, holds, then the next
// begins \u2014 so every curve's rise/fall can be read across a single component
// before moving on. Hover / click replays the sweep.
function EngineEnergyFigure({ play }) {
  const [xMax, setXMax] = useState(60)   // right edge of the drawn region
  const [seg, setSeg]   = useState(-1)   // component interval currently drawing
  ...
  const animate = useCallback(() => {
    ...
    if (prefersReduced()) { setXMax(470); setSeg(-1); setDone(true); return }

    const DRAW = 300, HOLD = 200          // ms to draw a component, then pause
    const nSeg = STATION_X.length - 1     // 8 components
    const span = DRAW + HOLD
    setDone(false)
    const t0 = performance.now()

    const tick = (now) => {
      const e = now - t0
      const s = Math.floor(e / span)
      if (s >= nSeg) { setXMax(470); setSeg(-1); setDone(true); return }
      const local = Math.min((e - s * span) / DRAW, 1)
      const ease = local < 0.5 ? 2 * local * local : 1 - Math.pow(-2 * local + 2, 2) / 2
      const x0 = STATION_X[s], x1 = STATION_X[s + 1]
      setSeg(s)
      setXMax(x0 + (x1 - x0) * ease)
      raf.current = requestAnimationFrame(tick)
    }
    raf.current = requestAnimationFrame(tick)
  }, [])

  // start when the figure is revealed, and again on every hover / click replay
  useEffect(() => {
    if (!play) { setXMax(60); setSeg(-1); setDone(false); return }
    animate()
    return () => cancelAnimationFrame(raf.current)
  }, [play, run, animate])

  const replay = () => { if (play) setRun(r => r + 1) }
  ...
  return (
    <svg viewBox="0 0 520 320" className="q1d-svg" aria-hidden
      ...
      {seg >= 0 && !done && (
        <rect className="q6-band" x={STATION_X[seg]} y="40"
          width={STATION_X[seg + 1] - STATION_X[seg]} height="240" />
      )}
      ...
      {STATIONS.map(([x, l], i) => {
        const active = !done && (i === seg || i === seg + 1)
        return (
          <g key={l}>
      ...
      {/* curves, each clipped to the current sweep position */}
      {ENGINE_CURVES.map((c) => (
        <path key={c.cls} className={c.cls} d={clipCurve(c.pts, xMax)} />
      ))}

      {/* thrust annotation appears once the nozzle spike is fully drawn */}
      {done && (
        <>
...
function Figure({ name, play }) {
  if (name === 'engine-energy') return <EngineEnergyFigure play={play} />
  return null
}
  ...
  return (
    <div className="slide-inner compress-slide">
      ...
      <figure className={`q1d-fig reveal-block${revealed >= figStep ? ' revealed' : ''}`}>
        <Figure name={slide.figure} play={revealed >= figStep} />
