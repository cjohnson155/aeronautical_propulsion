...
export const slides = [
  ...
  { type: 'pvwork', sectionNumber: 'Section 5 · 5.10',
    heading: 'Net Work Is the Signed Area Under the P–v Legs',
    intro: 'With δw = P dv in hand, net work is the area the cycle encloses. Shade the area under each leg — signed by whether the gas expands or is compressed — and watch the four contributions combine into the enclosed loop.' },
...
function PvWorkSlide({ revealed }) {
  ...
  const mp = mapper(0, vmax, 0, pmax)
  const K12 = P1 * Math.pow(st.v[0], G), K34 = st.P2 * Math.pow(st.v[2], G)
  const l12 = isentPV(K12, st.v[0], st.v[1])       // v1 → v2 (compression)
  const l21 = isentPV(K12, st.v[1], st.v[0])       // v2 → v1 (lower boundary)
  const l34 = isentPV(K34, st.v[2], st.v[3])       // v3 → v4 (expansion)
  const pp = ([v, P]) => `${mp.fx(v).toFixed(1)},${mp.fy(P).toFixed(1)}`
  const drop = (pts) => [...pts, [pts[pts.length - 1][0], 0], [pts[0][0], 0]]
  const poly = (pts) => pts.map(pp).join(' ')
  const area12 = poly(drop(l12))
  const area23 = poly(drop([[st.v[1], st.P2], [st.v[2], st.P2]]))
  const area34 = poly(drop(l34))
  const area41 = poly(drop([[st.v[3], P1], [st.v[0], P1]]))
  const areaUpper = poly(drop([[st.v[1], st.P2], [st.v[2], st.P2], ...l34.slice(1)]))
  const areaLower = poly(drop([...l21, [st.v[3], P1]]))
  const loop = poly([...l12, [st.v[2], st.P2], ...l34.slice(1)])
  const legPath = (pts) => toPath(pts, mp)
  const pts = st.v.map((v, i) => [mp.fx(v), mp.fy(st.P[i])])
  const off = [[-2, 16], [-8, -6], [4, -7], [8, 15]]

  const leg = (step) => r >= step && r <= 4
  const combined = r === 5
  const net = r >= 6
  const GREEN = '#5fd39a', RED = 'var(--bad)'
  const caps = [
  ...
  return (
    <div className="slide-inner compress-slide">
      ...
      <p className="cf-note cf-note--lead anim-in" style={{ marginBottom: 8 }}>
        The Gibbs form gave us <strong>δw = P dv</strong>, so every leg contributes ∫P dv. Legs where the gas
        expands add work <strong>out</strong>; legs where it is compressed take work <strong>in</strong>.
      </p>

      <div className="pv-wrap">
        <figure className="cyc-fig pv-card">
          <figcaption className="cyc-figc" style={{ color: net ? GREEN : 'var(--muted)' }}>P–v diagram · r_p = 8</figcaption>
          <svg viewBox="0 0 320 260" className="cyc-svg">
            <line x1={mp.x0} y1={mp.y0} x2={mp.x0} y2={mp.y0 + mp.h} className="ax" />
            <line x1={mp.x0} y1={mp.y0 + mp.h} x2={mp.x0 + mp.w} y2={mp.y0 + mp.h} className="ax" />
            <text x={mp.x0 - 6} y={mp.y0 + 2} className="axttl" textAnchor="end">P</text>
            <text x={mp.x0 + mp.w} y={mp.y0 + mp.h + 22} className="axttl" textAnchor="end">v (m³/kg)</text>

            {/* per-leg signed areas (steps 1–4) */}
            {leg(1) && <polygon points={area12} fill={RED} opacity="0.30" />}
            {leg(2) && <polygon points={area23} fill={GREEN} opacity="0.30" />}
            {leg(3) && <polygon points={area34} fill={GREEN} opacity="0.30" />}
            {leg(4) && <polygon points={area41} fill={RED} opacity="0.30" />}
            {/* combined boundaries (step 5) */}
            {combined && <polygon points={areaUpper} fill={GREEN} opacity="0.26" />}
            {combined && <polygon points={areaLower} fill={RED} opacity="0.34" />}
            {/* net enclosed loop (step 6) */}
            {net && <polygon points={loop} fill={GREEN} opacity="0.34" />}

            {/* loop outline */}
            <path d={legPath(l12)} className="proc" style={{ stroke: 'var(--ink)' }} />
            <path d={legPath([[st.v[1], st.P2], [st.v[2], st.P2]])} className="proc" style={{ stroke: 'var(--ink)' }} />
            <path d={legPath(l34)} className="proc" style={{ stroke: 'var(--ink)' }} />
            <path d={legPath([[st.v[3], P1], [st.v[0], P1]])} className="proc" style={{ stroke: 'var(--ink)' }} />

            {pts.map(([x, y], i) => (
              <g key={i}>
                <circle cx={x} cy={y} r="4" fill="var(--accent)" stroke="var(--bg)" strokeWidth="1.2" />
                <text x={x + off[i][0]} y={y + off[i][1]} className="ptxt">{i + 1}</text>
              </g>
            ))}
          </svg>
        </figure>
      ...
      <p className="cyc-cap" style={{ marginTop: 8 }}>
        <b style={{ color: net ? GREEN : 'var(--accent)' }}>{caps[Math.min(r, 6)]}</b>
      </p>
    </div>
  )
}
...
const STEP_COUNTS = { cycle: 8, pvwork: 6 }      // cycle: P–v ×4 then T–s ×4 · pvwork: ∫P dv build
...
:root{
  ...
  --accent:#5ec8d8; --accent-2:#f0a93b; --bad:#e2685c; --pos:#5fd39a; --rule:#27405e;
