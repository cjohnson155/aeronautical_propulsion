...
export const slides = [
  ...
  // ── ENERGY THROUGH THE ENGINE (figure) ──────────────────────────────────────
  {
    ...
    intro:
      'Every stage from freestream to nozzle, now as a picture. Follow each currency station by station \u2014 the shapes are the answer key to the exercise you just did.',
...
// ─── Figures ─────────────────────────────────────────────────────────────────
// Ten station planes, evenly spaced across x = 60..470 (gap = 410/9 ≈ 45.56).
// The leading "Free" plane is the freestream reference; every gap after it is
// exactly one component acting (the component named at the gap's RIGHT edge):
//   Free→Inlet = inlet (ram)   Inlet→Fan = fan   Fan→LPC / LPC→HPC = compression
//   HPC→Burner = combustion    Burner→HPT / HPT→LPT = expansion
//   LPT→A/B = afterburner      A/B→Nozzle = nozzle
const STATIONS = [
  [60, 'Free'], [105.56, 'Inlet'], [151.11, 'Fan'], [196.67, 'LPC'], [242.22, 'HPC'],
  [287.78, 'Burner'], [333.33, 'HPT'], [378.89, 'LPT'], [424.44, 'A/B'], [470, 'Nozzle'],
]
...
const SEGMENTS = STATION_X.length - 1        // components each currency crosses (9)
...
const ENGINE_CURVES = [
  // chem: zero everywhere except two combustion spikes. Each spike peaks right
  // where fuel is INJECTED (burner/A-B entry) and decays to ~0 by the exit
  // plane — mirroring the thermal RISE beside it, since the burn converts one
  // into the other (they trade off; they do not peak together).
  { key: 'chem',  cls: 'q6-chem',  pts: [[60,280],[242.22,280],[248,150],[258,190],[270,235],[287.78,280],[378.89,280],[385,168],[398,205],[411,242],[424.44,280],[470,280]] },
  { key: 'therm', cls: 'q6-therm', pts: [[60,236],[105.56,228],[151.11,218],[196.67,192],[242.22,142],[287.78,58],[333.33,118],[378.89,162],[424.44,105],[470,180]] },
  { key: 'mech',  cls: 'q6-mech',  pts: [[60,232],[105.56,212],[151.11,196],[196.67,165],[242.22,105],[287.78,110],[333.33,150],[378.89,192],[424.44,205],[470,235]] },
  // kin: high in the freestream, dropped by the inlet, ~flat through the core,
  // a modest BUBBLE through the turbines (stators accelerate the flow, rotors
  // take it back out), then the nozzle spike that makes thrust.
  { key: 'kin',   cls: 'q6-kin',   pts: [[60,170],[105.56,208],[151.11,206],[196.67,207],[242.22,205],[287.78,210],[333.33,188],[378.89,182],[424.44,202],[470,66]] },
]
...
// total clicks to draw every currency across every component (4 x 9 = 36)
...
function EngineEnergyFigure({ step }) {
  ...
  return (
    <svg viewBox="0 0 520 320" className="q1d-svg" aria-hidden>
      ...
      {extent.kin >= 470 && (
        <>
          <circle cx="470" cy="66" r="4" style={{ fill: C_KIN }} />
          <text x="464" y="57" className="q1d-t" style={{ fill: C_KIN, fontWeight: 600 }} textAnchor="end">thrust</text>
        </>
      )}
    </svg>
  )
